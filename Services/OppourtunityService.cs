...
namespace gasopper_crm_server.Services
{
    ...
    public class OpportunityService : IOpportunityService
    {
        ...
        {
            var query = BuildOpportunityQuery(currentUserId, currentUserRole);

            var opportunity = await query
        ...
        public async Task<OpportunityResponseDto?> UpdateOpportunityAsync(int opportunityId, UpdateOpportunityDto updateDto, int currentUserId, int currentUserRole)
        {
            ...
            if (opportunity == null || !CanAccessOpportunity(opportunity, currentUserId, currentUserRole))
                return null;
        ...
        private IQueryable<Opportunity> BuildOpportunityQuery(int currentUserId, int currentUserRole)
        {
            ...
            // Apply role-based filtering (same pattern as leads)
            ...
            else if (currentUserRole == 2) // Manager can see own + team opportunities
            {
                ...
                query = query.Where(o => o.assigned_to == currentUserId || teamMemberIds.Contains(o.assigned_to));
            }
        ...
        private bool CanAccessOpportunity(Opportunity opportunity, int currentUserId, int currentUserRole)
        {
            ...
            if (currentUserRole == 2) // Manager can access own + team
            {
                ...
                var assignedUser = _context.Users.Find(opportunity.assigned_to);
        ...
        private OpportunityResponseDto MapToOpportunityResponseDto(Opportunity opportunity)
        {
            ...
            return new OpportunityResponseDto
            {
                ...
                AssignedToName = $"{opportunity.AssignedToUser.first_name} {opportunity.AssignedToUser.last_name}",
                CreatedBy = opportunity.created_by,
                CreatedByName = $"{opportunity.CreatedByUser.first_name} {opportunity.CreatedByUser.last_name}",
